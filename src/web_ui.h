...
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'%3E%3Crect width='24' height='24' rx='5' fill='%231e88e5'/%3E%3Cpath fill='white' d='M15 13V5a3 3 0 0 0-6 0v8a5 5 0 1 0 6 0zm-3-9a1 1 0 0 1 1 1v8.5l.5.3a3 3 0 1 1-3 0l.5-.3V5a1 1 0 0 1 1-1z'/%3E%3Ccircle cx='12' cy='17' r='2' fill='white'/%3E%3C/svg%3E">
<title>HP-Remote WiFi Setup</title>
<style>
...
<input id="hostname" value="hp-remote">
...
/* Touch-Button-Layout wie am echten Display: 4 oben, 2 unten zentriert */
.touchRowTop{
  display:grid;
...
@media (max-width:600px){
  .touchRowTop{ grid-template-columns:repeat(2, 1fr); }
  .touchRowBottom{ max-width:100%; }
}
button,input,select{
  width:100%;
  box-sizing:border-box;
...
@media (max-width:1450px){
  .statusBar{grid-template-columns:repeat(5,minmax(120px,1fr))}
  .app{overflow-x:auto}
}
...
<header>
  <h1>HP-Remote</h1>
  <button type="button" id="btnToggleAll" class="collapseBtn" onclick="toggleAllSections()" title="Expand/collapse all sections">▾</button>
</header>
...
<div class="app">
  <div class="statusBar">
    ...
    <div class="statusItem">
      <div class="k">Room Sensor</div>
      <div class="v">
        <span id="led_room" class="statusLed"></span>
        <span id="st_room">-</span>
      </div>
    </div>
    ...
    <div class="statusItem">
      <div class="k">Room Temperature</div>
      <div class="v" id="st_room_temp">-</div>
    </div>
    ...
    <div class="statusItem">
      <div class="k">Room Humidity</div>
      <div class="v" id="st_room_humidity">-</div>
    </div>
  ...
  <div class="configSections">
    ...
    <div class="collapsibleSection">
      ...
      <div id="camera_content" class="collapsibleContent hidden">
        <div class="subtle" style="padding:4px 0">
          Board: <strong>ESP32-S3-CAM</strong> &nbsp;·&nbsp; Kamera: <strong>OV2640</strong>
        </div>
        ...
        <label>Fine Rotation: <span id="cam_fine_rotation_val">0.0</span>°
          <input id="cam_fine_rotation" type="range" min="-5" max="5" step="0.1" value="0"
                 oninput="onFineRotationInput()">
        </label>
        <div class="small">Fine adjustment of the camera angle (±5°). Affects live preview and OCR analysis.</div>
    ...
    <div class="collapsibleSection">
      ...
      <div id="mqtt_content" class="collapsibleContent hidden">
        ...
        <div class="card">
          <h3>HA-Discovery</h3>
          <label><input id="ha_enabled" type="checkbox" onchange="syncHaDiscoveryFields()">Enable Home Assistant Auto-Discovery</label>
          <div class="small">When disabled, no HA discovery messages are sent. Plain MQTT data (display values, touch topics) keeps working.</div>
          <label>Discovery Prefix <input id="ha_prefix"></label>
          <label>Device Name <input id="d_name" placeholder="HP-Remote"></label>
          <div class="small">Device name in Home Assistant. Reset automatically on config import and can be adjusted here.</div>
        </div>
      </div>
    </div>


    <!-- Touch-Controller (HP-Remote) -->
    <div class="collapsibleSection">
      <div class="collapsibleHeader">
        <div class="sectionTitle">
          <button type="button" class="collapseBtn" id="touch_btn"
            onclick="toggleSection('touch_content','touch_btn')">▸</button>
          <h3>Touch-Controller</h3>
        </div>
      </div>

      <div id="touch_content" class="collapsibleContent hidden">
        <div class="subtle">Simulate capacitive button presses. Control via MQTT / Home Assistant in operation.</div>

        <!-- Obere Reihe: 4 Tasten (wie am echten Display) -->
        <div class="touchRowTop">
          <button onclick="touchPress('power')">&#9211; Power</button>
          <button onclick="touchPress('boost')">&#9889; Boost / PV</button>
          <button onclick="touchPress('settings')">&#9881; Settings</button>
          <button onclick="touchPress('time')">&#128336; Time</button>
        </div>

        <!-- Untere Reihe: 2 Tasten, zentriert -->
        <div class="touchRowBottom">
          <button onclick="touchPress('up')">&#8743; Temp+</button>
          <button onclick="touchPress('down')">&#8744; Temp&minus;</button>
        </div>

        <!-- Funktions-Shortcuts (lange Drücke / Kombinationen) -->
        <div class="subtle" style="margin-top:14px">Function shortcuts (long-press / combos):</div>
        <div class="touchRowTop" style="margin-top:6px">
          <button onclick="touchShortcut('standby')">&#9211; Standby</button>
          <button onclick="touchShortcut('fan')">&#10052; Fan vent</button>
          <button onclick="touchShortcut('lock')">&#128274; Key lock</button>
          <button onclick="touchShortcut('timer')">&#128336; Timer setup</button>
        </div>

        <div id="touch_result" class="subtle" style="margin-top:8px"></div>

        <!-- I2C-Bus-Diagnose: findet PCF8574, SHT31 und sonstige Geräte am gemeinsamen Bus -->
        <div class="subtle" style="margin-top:14px">I2C bus diagnostics (touch controller + room sensor share the same bus):</div>
        <button type="button" style="margin-top:6px" onclick="i2cScan()">Scan I2C bus</button>
        <div id="i2c_result" class="subtle" style="margin-top:8px"></div>
      </div>
    </div>

    <!-- Room Sensor (SHT31, optional) -->
    <div class="collapsibleSection">
      <div class="collapsibleHeader">
        <div class="sectionTitle">
          <button type="button" class="collapseBtn" id="room_btn"
            onclick="toggleSection('room_content','room_btn')">▸</button>
          <h3>Room Sensor</h3>
        </div>

        <label class="collapsibleToggle">
          <input id="room_enabled" type="checkbox">
          <span>Enable</span>
        </label>
      </div>

      <div id="room_content" class="collapsibleContent hidden">
        <div class="subtle" style="padding:4px 0">
          Optional SHT31 sensor (I2C, shares the bus with the touch controller) for
          temperature/humidity in the boiler room. If no sensor is detected, MQTT values
          and Home Assistant discovery stay off automatically – see status above ("Room Sensor").
        </div>

        <div class="two">
          <label>Temp Offset (°C) <input id="room_temp_offset" type="number" step="0.1" min="-10" max="10"></label>
          <label>Humidity Offset (%) <input id="room_humidity_offset" type="number" step="0.1" min="-20" max="20"></label>
        </div>

        <label>Read Interval (s) <input id="room_interval" type="number" min="5" max="3600"></label>
      </div>
    </div>
  </div>
    ...
    <div class="sidebar">
      ...
      <div class="collapsibleSection">
        ...
        <div id="roi_content" class="collapsibleContent hidden">
          <label style="background:#2a2a18;padding:6px 8px;border-radius:6px;border:1px solid #5a5a28">
            <input id="f_locked" type="checkbox" onchange="onLockToggle()">
            🔒 ROI locked (prevents accidental changes)
          </label>
          <label>ID <input id="f_id"></label>
          <label>Label <input id="f_label" oninput="updateSelectedFromForm()"></label>
          ...
          <label id="f_auto_threshold_row"><input id="f_auto_threshold" type="checkbox" onchange="syncAutoThreshold()">Auto Threshold</label>
          ...
          <div id="ha_global_hint" class="small" style="display:none;color:#c80">⚠ Home Assistant Auto-Discovery is globally disabled (MQTT settings). This toggle currently has no effect.</div>
      ...
      <div class="collapsibleSection">
        ...
        <div id="debug_content" class="collapsibleContent hidden">
          ...
          <button id="btnWifiReset" onclick="wifiReset()" style="margin-top:12px;background:#7a3030">Reset WiFi (Setup AP)</button>
          <div class="subtle">Clears the WiFi credentials and restarts into the <b>HP-Remote-Setup</b> access point so you can connect to a different network. All other settings (MQTT, ROIs) are kept.</div>
        </div>
      </div>
    </div>
  </div>
...
function syncHaFields(){
  ...
  // Globaler HA-Schalter aus → ROI-Checkbox ausgrauen + Hinweis zeigen
  const globalOn = state.config?.ha?.enabled !== false;
  const cb = document.getElementById('f_ha_enabled');
  const hint = document.getElementById('ha_global_hint');
  if (cb) cb.disabled = !globalOn;
  if (hint) hint.style.display = globalOn ? 'none' : 'block';
}
...
async function loadStatus(){
  ...
  try {
    ...
    return;  // Interval läuft weiter, nächster Versuch in 3s
  }
  try {
  ...
  document.getElementById('st_cam_model').textContent = 'ESP32-S3-CAM';
  ...
  setLed("led_room", !!s.room?.available);

  document.getElementById('st_room').textContent =
    s.room?.available ? 'Detected' : 'Not detected';
  document.getElementById('st_room_temp').textContent =
    (s.room?.available && s.room?.temp_c != null) ? Number(s.room.temp_c).toFixed(1) + ' °C' : '-';
  document.getElementById('st_room_humidity').textContent =
    (s.room?.available && s.room?.humidity_pct != null) ? Number(s.room.humidity_pct).toFixed(0) + ' %' : '-';
  ...
  if (s.ota?.running) {
    ...
    console.warn('loadStatus render error:', e);
  }
}

async function loadConfig(){
  state.config = await api('/api/config');
  try { fillPanels(); } catch(e) { console.warn('fillPanels:', e); }
  try { renderList(); } catch(e) { console.warn('renderList:', e); }
}
...
function openLiveStream(){
  if (!document.getElementById('stream_enabled').checked) {
    setOutText('Live Stream is disabled.\nEnable "🎥 Live Stream" in the Camera section and save the config.');
    return;
  }
...
async function wifiReset(){
  const confirmed = confirm(
    'WiFi-Zugangsdaten wirklich löschen?\\n\\n' +
    'Das Gerät startet danach im Setup-AP "HP-Remote-Setup". ' +
    'Verbinde dich mit diesem AP und öffne 192.168.4.1, um ein neues WLAN einzurichten.\\n\\n' +
    'MQTT- und ROI-Einstellungen bleiben erhalten.'
  );
  if (!confirmed) return;

  setButtonBusy('btnWifiReset', true, 'Resetting...');

  try {
    const res = await api('/api/wifi_reset', { method:'POST' });
    document.getElementById('out').textContent =
      JSON.stringify(res, null, 2) +
      "\\n\\nGerät startet im Setup-AP 'HP-Remote-Setup'.\\n" +
      "Mit diesem AP verbinden und 192.168.4.1 öffnen.";
    setButtonBusy('btnWifiReset', false);
    flashButtonState('btnWifiReset', true, '✓ AP-Modus');
  } catch (e) {
    setButtonBusy('btnWifiReset', false);
    flashButtonState('btnWifiReset', false);
    setOutText(String(e));
  }
}
...
function renderList(){
  ...
  state.config.rois.forEach(roi => {
    ...
    const lockIcon = roi.locked ? '🔒 ' : '';
    const displayName = (roi.label && roi.label.trim()) ? roi.label : roi.id;
    li.textContent = `${lockIcon}${displayName} (${roi.type}${roi.type === 'sevenseg' ? ':' + roi.digits + (roi.decimal_places > 0 ? ',dp' + roi.decimal_places : '') : ''})`;
...
function fillForm(roi){
  document.getElementById('f_locked').checked = !!roi.locked;
  ...
  applyLockState(!!roi.locked);
}

// Graut alle ROI-Editor-Felder aus wenn gesperrt (außer der Lock-Checkbox selbst)
function applyLockState(locked){
  const content = document.getElementById('roi_content');
  if (!content) return;
  const inputs = content.querySelectorAll('input, select, button');
  inputs.forEach(el => {
    if (el.id === 'f_locked') return;   // Lock-Checkbox bleibt bedienbar
    el.disabled = locked;
  });
  content.style.opacity = locked ? '0.55' : '1';
}

function onLockToggle(){
  const locked = document.getElementById('f_locked').checked;
  const roi = selectedRoi();
  if (roi) roi.locked = locked;
  applyLockState(locked);
  renderList();
  // Sperre sofort & direkt persistieren (ohne updateSelectedFromForm,
  // das bei disabled Feldern stören könnte)
  saveConfigDirect();
}

// Speichert state.config direkt ohne erneutes Einlesen der Formularfelder
...
function fillPanels(){
  // Defensive: alle erwarteten Config-Objekte sicherstellen
  if (!state.config) state.config = {};
  if (!state.config.camera) state.config.camera = {};
  if (!state.config.mqtt) state.config.mqtt = {};
  ...
  if (!Array.isArray(state.config.rois)) state.config.rois = [];
  ...
  {
    const fr = (state.config.camera.fine_rotation ?? 0);
    document.getElementById('cam_fine_rotation').value = fr;
    document.getElementById('cam_fine_rotation_val').textContent = Number(fr).toFixed(1);
    applyPreviewRotation(fr);
  }
  ...
  document.getElementById('ha_enabled').checked = state.config.ha?.enabled !== false;
  document.getElementById('ha_prefix').value = state.config.ha?.discovery_prefix || 'homeassistant';
  syncHaDiscoveryFields();
  ...
  if (!state.config.room_sensor) state.config.room_sensor = {};
  document.getElementById('room_enabled').checked = state.config.room_sensor.enabled !== false;
  document.getElementById('room_temp_offset').value = state.config.room_sensor.temp_offset ?? 0;
  document.getElementById('room_humidity_offset').value = state.config.room_sensor.humidity_offset ?? 0;
  document.getElementById('room_interval').value = state.config.room_sensor.read_interval_sec ?? 60;
  ...
  setSectionExpanded('room_content','room_btn', false);
}
...
function updatePanelsFromForm(){
  ...
  state.config.camera.fine_rotation = parseFloat(document.getElementById('cam_fine_rotation').value) || 0;
  ...
  if (!state.config.ha) state.config.ha = {};
  state.config.ha.enabled = document.getElementById('ha_enabled').checked;
  ...
  if (!state.config.room_sensor) state.config.room_sensor = {};
  state.config.room_sensor.enabled = document.getElementById('room_enabled').checked;
  state.config.room_sensor.temp_offset = parseFloat(document.getElementById('room_temp_offset').value) || 0;
  state.config.room_sensor.humidity_offset = parseFloat(document.getElementById('room_humidity_offset').value) || 0;
  state.config.room_sensor.read_interval_sec = parseInt(document.getElementById('room_interval').value || '60', 10);
...
function updateSelectedFromForm(){
  ...
  // Lock-Status immer aktualisieren (auch um zu entsperren)
  roi.locked = document.getElementById('f_locked').checked;
  // Bei gesperrtem ROI keine weiteren Felder übernehmen
  if (roi.locked) {
    renderList();
    return;
  }
  ...
  // Auto-Threshold nur für sevenseg; bei Symbolen immer feste Schwellwerte
  roi.auto_threshold = (roi.type === 'symbol')
    ? false
    : document.getElementById('f_auto_threshold').checked;
...
function syncTypeFields(){
  ...
  // Auto-Threshold ergibt nur bei sevenseg Sinn. Bei Symbolen ausblenden
  // und erzwingen dass es aus ist (feste Schwellwerte mit Hysterese sind
  // für selbstleuchtende An/Aus-Symbole die richtige Wahl).
  const autoRow = document.getElementById('f_auto_threshold_row');
  if (type === 'symbol') {
    document.getElementById('f_auto_threshold').checked = false;
...
const SEG_STANDARD = [
  [0.36,0.06,0.30,0.06],[0.80,0.18,0.10,0.20],[0.80,0.62,0.10,0.20],
  [0.36,0.88,0.30,0.06],[0.12,0.62,0.10,0.20],[0.12,0.18,0.10,0.20],
  [0.36,0.47,0.30,0.06]
];
...
function renderSegProfList() {
  ...
  segProfiles().forEach(p => {
    ...
    o.value = p.name;
    o.textContent = p.name + (p.name === 'standard' ? ' (read-only)' : '');
    sel.appendChild(o);
  });
...
function beginDrag(ev){
  ...
  // Gesperrtes ROI: nur auswählen, nicht verschieben
  if (roi.locked) {
    ev.preventDefault();
    return;
  }
...
function draw(){
  ...
  state.config.rois.forEach(roi => {
    ...
    const suffix = roi.type === 'sevenseg'
      ...
      : '';
    const nameText = (roi.label && roi.label.trim()) ? roi.label : roi.id;
    const lockPrefix = roi.locked ? '🔒 ' : '';
    const labelText = lockPrefix + nameText + suffix;
...
async function exportConfig(){
  ...
  try {
    ...
    a.href = url;
    a.download = 'HP-Remote-config.json';
...
[
  'f_id','f_label','f_locked','f_ha_enabled','f_ha_name','f_ha_unit','f_ha_icon','f_ha_device_class','f_ha_state_class',
  ...
  'cam_frame','cam_jpeg_quality',
  'cam_auto_exposure','cam_auto_gain','cam_auto_whitebalance','cam_vflip','cam_hflip','cam_fine_rotation','cam_aec_value','cam_agc_gain',
  'm_image_enabled',
  'stream_enabled',
...
(async function init(){
  ...
  // Auto-Refresh sofort starten – unabhängig von Config/Snapshot
  setInterval(loadStatus, 3000);

  try { await loadStatus();  } catch(e) { console.warn('init loadStatus:', e); }
  try { await loadConfig();  } catch(e) { console.warn('init loadConfig:', e); }
  try { await loadSnapshot(); } catch(e) { console.warn('init loadSnapshot:', e); }
})();

function applyPreviewRotation(deg){
  // Nur das Kamerabild drehen – das Overlay-Canvas (ROI-Rahmen/Segmente)
  // bleibt unrotiert, damit die ROIs an fester Bildposition bleiben.
  const img = document.getElementById('snap');
  if (img) {
    img.style.transform = 'rotate(' + (deg || 0) + 'deg)';
    img.style.transformOrigin = 'center center';
  }
}

function onFineRotationInput(){
  const v = parseFloat(document.getElementById('cam_fine_rotation').value) || 0;
  document.getElementById('cam_fine_rotation_val').textContent = v.toFixed(1);
  applyPreviewRotation(v);
}

function syncHaDiscoveryFields(){
  const on = document.getElementById('ha_enabled').checked;
  document.getElementById('ha_prefix').disabled = !on;
  document.getElementById('d_name').disabled = !on;
  // state.config aktuell halten, damit ROI-Editor den globalen Status kennt
  if (state.config && state.config.ha) state.config.ha.enabled = on;
  // ROI-Editor Checkbox-Status mit aktualisieren falls sichtbar
  if (document.getElementById('f_ha_enabled')) syncHaFields();
}

async function touchPress(button) {
  const el = document.getElementById('touch_result');
  el.textContent = 'Sending: ' + button + '...';
  try {
    ...
    el.textContent = res && res.ok ? ('OK: ' + button) : 'Error';
  } catch(e) {
    el.textContent = 'Error: ' + String(e);
  }
}

async function touchShortcut(name) {
  const el = document.getElementById('touch_result');
  el.textContent = 'Sending shortcut: ' + name + '...';
  try {
    const res = await api('/api/touch', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
    ...
    el.textContent = res && res.ok ? ('OK: ' + name) : 'Error';
  } catch(e) {
    el.textContent = 'Error: ' + String(e);
  }
}
...
async function i2cScan() {
  ...
  try {
    const res = await api('/api/i2c/scan', { method: 'POST' });
    if (!res || !res.ok) { el.textContent = 'Error'; return; }
    const devices = res.devices || [];
    if (devices.length === 0) { el.textContent = 'No I2C devices found'; return; }
    el.textContent = devices.map(d => d.address + (d.name ? ' – ' + d.name : ' – unknown')).join(', ');
  } catch(e) {
    el.textContent = 'Error: ' + String(e);
  }
}
